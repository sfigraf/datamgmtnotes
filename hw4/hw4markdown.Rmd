...
head <- transportation_costs %>%
  head(10)
head1 <- as.table(as.matrix(head))


```
```{r, include = TRUE}
kable(head1,row.names = FALSE)
```
...
```
Here is the final table.It is cheaper next year to use supplier B. 
```{r}
...
In the next year, here are the projected costs of transportation for each store, using the cheapest carrier each week.
It is usually cheaper to use Carrier X, especially to store 1, but not always cheaper to use it for store B. 
```{r}
x <- supplierB %>%
